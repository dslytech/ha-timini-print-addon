...
<div class="card">
  ...
  <p class="hint">If you have more than one printer nearby, scan first
  - both cards below get a printer picker populated from the results,
  so you choose which one to send each print job to.</p>
</div>
...
<div class="card print">
  ...
  <label for="printer-model-text-input">Force model (optional, e.g. for "manual model required" printers)</label>
  <input type="text" id="printer-model-text-input" placeholder="leave blank unless needed"
...
<div class="card print">
  ...
  <label for="printer-select-img">Printer</label>
  <select id="printer-select-img">
    <option value="">Auto (first supported printer found - scan to pick a specific one)</option>
  </select>
  <label for="printer-model-image-input">Force model (optional, e.g. for "manual model required" printers)</label>
  <input type="text" id="printer-model-image-input" placeholder="leave blank unless needed"
    style="width:100%; margin-bottom:.8rem;">
...
function populatePrinterSelect(scanStdout) {
    for (const selectId of ['printer-select', 'printer-select-img']) {
...
async function doPrint() {
    ...
    const printerModel = document.getElementById('printer-model-text-input').value;
...
async function doPrintImage() {
    ...
    const printer = document.getElementById('printer-select-img').value;
    const state = window.timiniImageState;
    ...
    const send = (base64, filename) => {
        ...
        if (darkness) body.darkness = parseInt(darkness, 10);
        const printerModel = document.getElementById('printer-model-image-input').value;
        if (printerModel) body.printer_model = printerModel;
        fetch('/print_image', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_print_text(self):
        ...
        result = run_cli(args, timeout=90)
    ...
    def _handle_print_image(self):
        ...
        try:
            ...
            printer = data.get("printer") or PRINTER_NAME
            printer_model = data.get("printer_model")
            result = self._print_local_file(tmp_path, printer, timeout=120, darkness=darkness, printer_model=printer_model)
            status = 200 if result.get("returncode") == 0 else 500
            self._send_json(status, result)
        finally:
            try:
