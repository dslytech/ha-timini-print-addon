...
# TiMini-Print's own CLI has no --copies flag, so this is done
# client-side (in this wrapper) by simply calling the CLI that many
# times in a row. Capped to a sane number to avoid an accidental huge
# value tying up the printer/paper roll for a very long time.
MAX_COPIES = 20
...
<h1 style="display:flex; align-items:center; gap:.5rem;">
  TiMini Print
  <button onclick="toggleLicenses()" title="Show licenses"
    style="width:26px; height:26px; border-radius:50%; padding:0; font-weight:bold; font-style:italic; font-family:Georgia, serif; line-height:1;">i</button>
</h1>
...
without SSH.</p>
<pre id="licenses-panel" style="display:none;"></pre>
...
<div class="card">
  ...
  <label style="font-size:.85em; color:var(--muted); display:block; margin-top:.6rem;">Paper motion (each press moves a fixed step - there's no way to set a distance)</label>
  <div style="display:flex; gap:.6rem; margin-top:.3rem; align-items:center;">
    <button onclick="doPaperMotion('retract')" title="Retract paper"
      style="width:44px; height:44px; font-size:1.3em; font-weight:bold; padding:0;">−</button>
    <button onclick="doPaperMotion('feed')" title="Feed paper"
      style="width:44px; height:44px; font-size:1.3em; font-weight:bold; padding:0;">+</button>
    <span style="font-size:.85em; color:var(--muted);">Paper</span>
  </div>
  <pre id="paper-motion-result"></pre>
</div>
...
<div class="card print">
  ...
  <input type="number" id="darkness-text-input" min="1" max="5" step="1" value="3"
    style="width:100%; margin-bottom:.8rem;">
  <label for="copies-text-input">Copies</label>
  <input type="number" id="copies-text-input" min="1" max="20" step="1" value="1"
    style="width:100%; margin-bottom:.8rem;">
  <label style="display:flex; align-items:center; gap:.4rem; font-size:.85em; color:var(--muted); margin-bottom:.8rem; cursor:pointer;">
    <input type="checkbox" id="hard-wrap-checkbox">
    Hard-wrap by width (disable whitespace word wrapping)
  </label>
...
<div class="card print">
  ...
  <input type="number" id="darkness-image-input" min="1" max="5" step="1" value="3"
    style="width:100%; margin-bottom:.8rem;">
  <label for="copies-image-input">Copies</label>
  <input type="number" id="copies-image-input" min="1" max="20" step="1" value="1"
    style="width:100%; margin-bottom:.8rem;">
  <label for="pdf-pages-input">PDF pages (e.g. "1,3-5" - leave blank for all)</label>
  <input type="text" id="pdf-pages-input" placeholder="all pages" style="width:100%; margin-bottom:.8rem;">
  <label for="page-gap-input">Gap between pages (mm)</label>
  <input type="number" id="page-gap-input" min="0" step="1" placeholder="5 (default)" style="width:100%; margin-bottom:.4rem;">
  <label style="display:flex; align-items:center; gap:.4rem; font-size:.85em; color:var(--muted); cursor:pointer;">
    <input type="checkbox" id="trim-side-checkbox" checked>
    Trim white side margins
  </label>
  <label style="display:flex; align-items:center; gap:.4rem; font-size:.85em; color:var(--muted); margin-bottom:.8rem; cursor:pointer;">
    <input type="checkbox" id="trim-topbottom-checkbox" checked>
...
async function toggleLicenses() {
    ...
    const r = await fetch('/licenses');
    const j = await r.json();
    panel.innerText = j.stdout || j.stderr || '(no license text returned)';
    licensesLoaded = true;
}

async function doPaperMotion(action) {
    const resultEl = document.getElementById('paper-motion-result');
    resultEl.innerText = (action === 'feed' ? 'Feeding...' : 'Retracting...');
    ...
    const r = await fetch('/paper_motion', {
        method: 'POST',
...
async function doPrint() {
    ...
    const printerModel = document.getElementById('printer-model-input').value;
    if (printerModel) body.printer_model = printerModel;
    if (document.getElementById('hard-wrap-checkbox').checked) body.hard_wrap = true;
    const copiesText = document.getElementById('copies-text-input').value;
    if (copiesText) body.copies = parseInt(copiesText, 10);
...
async function doPrintImage() {
    ...
    const send = (base64, filename) => {
        ...
        const printerModel = document.getElementById('printer-model-input').value;
        if (printerModel) body.printer_model = printerModel;
        const pdfPages = document.getElementById('pdf-pages-input').value;
        if (pdfPages) body.pdf_pages = pdfPages;
        const pageGap = document.getElementById('page-gap-input').value;
        if (pageGap) body.page_gap = parseInt(pageGap, 10);
        body.trim_side_margins = document.getElementById('trim-side-checkbox').checked;
        body.trim_top_bottom_margins = document.getElementById('trim-topbottom-checkbox').checked;
        const copiesImage = document.getElementById('copies-image-input').value;
        if (copiesImage) body.copies = parseInt(copiesImage, 10);
        fetch('/print_image', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify(body)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):    # pylint: disable=invalid-name
        ...
        if self.path == "/licenses":
            result = run_cli(["--licenses"], timeout=15)
            self._send_json(200, result)
            return
    ...
    def do_POST(self):    # pylint: disable=invalid-name
        ...
        if self.path == "/print_image":
            ...
            return
        if self.path == "/paper_motion":
            self._handle_paper_motion()
            return
        self._send_json(404, {"error": "not found"})

    def _handle_paper_motion(self):
        ...
        if action not in ("feed", "retract"):
            self._send_json(400, {"error": "'action' must be 'feed' or 'retract'"})
            return
    ...
    def _handle_print_text(self):
        ...
        if data.get("hard_wrap"):
            args = ["--text-hard-wrap"] + args
        ...
        try:
            ...
            self._send_json(400, {"error": "'copies' must be a number"})
            return
        if not 1 <= copies <= MAX_COPIES:
            self._send_json(400, {"error": f"'copies' must be between 1 and {MAX_COPIES}"})
            return
    ...
    def _print_local_file(
        ...
        args = []
        if not trim_side_margins:
            args = ["--no-trim-side-margins"] + args
        if not trim_top_bottom_margins:
            args = ["--no-trim-top-bottom-margins"] + args
        if page_gap not in (None, ""):
            args = ["--page-gap", str(page_gap)] + args
        if pdf_pages:
            args = ["--pdf-pages", str(pdf_pages)] + args
        if printer_model:
            args = ["--printer-model", str(printer_model)] + args
        if darkness not in (None, ""):
            args = ["--darkness", str(darkness)] + args
    ...
    def _handle_print_image(self):
        ...
        page_gap = data.get("page_gap")
        if page_gap not in (None, ""):
            try:
                page_gap = int(page_gap)
            except (TypeError, ValueError):
                self._send_json(400, {"error": "'page_gap' must be a number"})
                return
            if page_gap < 0:
                self._send_json(400, {"error": "'page_gap' must be >= 0"})
                return
        else:
            page_gap = None

        pdf_pages = data.get("pdf_pages") or None
        trim_side_margins = data.get("trim_side_margins", True)
        trim_top_bottom_margins = data.get("trim_top_bottom_margins", True)
