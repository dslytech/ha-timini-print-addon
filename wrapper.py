...
class Handler(BaseHTTPRequestHandler):
    ...
    @staticmethod
    def _print_local_file(tmp_path: str, printer: str | None, timeout: int, darkness=None):
        args = []
        if darkness not in (None, ""):
            args = ["--darkness", str(darkness)] + args
        if printer:
    ...
    def _handle_print_image(self):
        ...
        try:
            with open(tmp_path, "wb") as f:
                f.write(image_bytes)
            printer = data.get("printer") or PRINTER_NAME
            result = self._print_local_file(tmp_path, printer, timeout=120, darkness=darkness)
            status = 200 if result.get("returncode") == 0 else 500
            self._send_json(status, result)
        finally:
            try:
